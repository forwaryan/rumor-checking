...
@pytest.fixture()
def pipeline(stable_test_env, monkeypatch) -> AnalyzePipeline:
    monkeypatch.setenv("LLM_MODEL", "model-primary")
    monkeypatch.setenv("LLM_MODELS", "model-primary,model-secondary")
    get_settings.cache_clear()
    return AnalyzePipeline()


def test_no_pick_leaves_override_none_so_failover_stays_enabled(pipeline: AnalyzePipeline):
...
def test_invalid_pick_leaves_override_none(pipeline: AnalyzePipeline):
    assert _apply(pipeline, "not-a-real-model") is None
    assert _apply(pipeline, "") is None


def test_valid_explicit_pick_pins_and_disables_failover(pipeline: AnalyzePipeline):
...
def test_override_applied_to_both_reasoner_and_enricher(pipeline: AnalyzePipeline):
    chosen = pipeline.settings.available_models[-1]
