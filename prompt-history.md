...
## 2026-03-15 02:24 [log]
- 用户补充了一个新的失败样例：`晨星生物是不是裁员了？`
- 该失败样例的外部回答模式不是“直接编造已裁员”，而是先把主体猜向金融公司 `Morningstar`，再混入“生物医药行业 2023-2024 裁员潮”和 `Morningstar` 旧闻，最后以低信心“无法确认”收口。
- 这说明当前问题不只是在 `safe / partial / complete` 模式漂移，还包括 `1)` 主体锚定不稳，`2)` 检索结果约束不足，`3)` 不相关背景知识被拼接进最终回答。
- 该失败应直接回提到 `T1.2`、`T3.1`、`T6.1`、`T7.1`：
  - `T1.2`：问题输入阶段需要更强的实体与公司名锚定，不能在主体未确认时自行猜测同名公司。
