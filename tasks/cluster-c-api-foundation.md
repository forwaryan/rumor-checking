...
你的目标是继续推进后端主链路，优先完成本文件中“进行中/未完成”的子任务；当前 `C10` 与 `C11` 第一阶段已完成，默认只处理 `C9` 残余质量问题、`F2/F4/F6` 回归反馈和文档/测试收尾。
...
- 当前状态说明：`C1` 到 `C8`、`C10` 和 `C11` 第一阶段已完成；`C9` 主体已完成但仍保留少量 provider 质量验证尾项；当前主线不再是“把主链打通”，而是处理 `F2/F4/F6/F8` 对主链稳定性的反馈。
- 当前前端已经能优先调用真实 `POST /api/v1/analyze`，且后端主链已经完成第一阶段去占位与 provenance 冻结；但输入、verdict、report mode 和随机 case 验收还没有完全收口，因此仍不能把“主链已能跑”直接等同于“任意新闻都已稳定较真”。
...
状态：已完成（第一阶段：主链去占位 + provenance 冻结）
...
本轮执行任务：
- 盘点并削弱 `AnalyzePipeline`、`VerdictEngine`、`TimelineBuilder`、`ReportBuilder` 对 `scenario_library`、模板 evidence 和 mock timeline 的硬依赖。
- 为 analyze 主链补充真实路径、回退路径和无证据路径的显式 provenance，冻结给 `E9` 的消费边界。
- 让 report 的 mode / verdict / timeline 更真实地消费输入、provider 输出和检索证据，同时保留 `safe_mode / partial_mode` 的保守降级。
- 补齐主链回归测试，并明确交给 `E9` 与 `F8` 的字段、判定口径和残余边界。

执行步骤：
- 继续盘点 `retrieval`、`input_normalizer`、`claim_extractor` 与 `scenario_library` 的主链接入点，确定真实路径与 fallback 路径的分叉条件。
- 设计并冻结 `Report` provenance schema，明确后端真实分析、后端 mock / replay、demo payload 与 frontend fallback 的区分口径。
- 重构 `AnalyzePipeline`、`VerdictEngine`、`TimelineBuilder`、`ReportBuilder`，优先消费真实输入、provider 输出和检索 evidence，并把场景模板限制在受控 fallback。
- 补 API 级与服务级测试，覆盖真实检索路径、mock / fallback 路径、无证据保守判定和 provenance 字段断言。
- 运行定向回归，最后回写 `C11` 完成记录、验证结果，以及给 `E9` / `F8` 的交接说明。

完成记录：
- 改动文件：`tasks/cluster-c-api-foundation.md`、`backend/app/api/v1/endpoints/analyze.py`、`backend/app/models/schemas.py`、`backend/app/services/analyze_pipeline.py`、`backend/app/services/input_normalizer.py`、`backend/app/services/claim_extractor.py`、`backend/app/services/provider_enricher.py`、`backend/app/services/verdict_engine.py`、`backend/app/services/timeline_builder.py`、`backend/app/services/report_builder.py`、`backend/tests/conftest.py`、`backend/tests/test_api.py`、`backend/tests/test_retrieval.py`。
- 完成方式：把 `scenario_library` 从 analyze 主链的硬依赖里降下来了；`InputNormalizer` 改为优先消费真实输入 / URL 抽取结果，`ClaimExtractor` 改为规则抽取 + provider 补强，`VerdictEngine` 只再吃检索证据或请求注入 mock evidence，不再回退到模板 evidence；`TimelineBuilder` 只再产出 retrieval timeline 或 input seed，不再回退到 mock timeline；`Report` 新增冻结后的 `provenance` 字段，并在 `AnalyzePipeline` 中显式区分 backend live、backend mock、backend replay、provider 使用、fallback 与 evidence/timeline 来源；同时把 analyze endpoint 改成按请求实例化 pipeline，避免本地真实 Kimi key 污染测试或联调结果。
- 验证结果：`pytest backend/tests/test_api.py backend/tests/test_retrieval.py -q` 通过，`31 passed`；已覆盖 mock retrieval 主链、真实 retrieval 主链、real->mock fallback、URL fallback、无证据 safe mode，以及 `provenance` 字段断言。
- 交给 `E9` 的字段边界：后端现在稳定输出 `report.provenance.source_type`、`event_source`、`claim_source`、`evidence_source`、`timeline_source`、`retrieval_provider`、`retrieval_cache_status`、`provider_used`、`fallback_used`、`fallback_reasons`；其中后端当前只会发 `backend_live / backend_mock / backend_replay`，`demo_payload / frontend_fallback` 由前端本地状态补齐，不应伪装成后端返回。
- 交给 `F8` 的验证口径：把 `source_type=backend_live` 且 `evidence_source=retrieval_live` 视为真实后端 + 真实检索路径；把 `source_type=backend_mock` 或 `evidence_source=retrieval_mock / request_mock` 视为 mock/replay/联调路径；把 `evidence_source=none` 或 `fallback_used=true` 视为保守降级路径，不能当作“已较真成功”的通过样本。
- 剩余问题：当前主链已经不再依赖场景模板证据和模板时间线，但 `InputNormalizer` / `ClaimExtractor` 仍是启发式规则，未命中检索样本的随机文本仍会停在 `safe_mode + input_seed`；`F8` 还需要在真实检索 provider 打开时补随机 case 验收，确认 live 路径下的 mode 分布与 provenance 分布符合预期；`E9` 仍需按上述 frozen 字段把 backend live、backend mock、demo payload 和 frontend fallback 明确展示出来。
