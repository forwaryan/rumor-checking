...
- 当前状态说明：`C1` 到 `C8` 已完成最小可运行 mock 版本；`C9`、`C10` 尚未开始。
...
实现备注：已完成，当前基于规则和 `scenario_library`。
...
实现备注：已完成，但字段仍需后续与 `contracts/` 冻结 schema 再对齐。
...
- 对异常链路返回统一的错误或保守结果。
实现备注：已完成，已通过 API smoke test 和 case 测试。
