...
- 剩余问题：`C9` 的“能调用真实 Kimi”已打通，但“输出质量调优”仍未完成；当前可见问题包括标题质量一般、claim 仍偏保守。若要让“较真”新闻应用在完整性上继续提升，下一步应继续做小样本验收、prompt 调优，以及与 `Cluster-D / C10 / F` 的联动收口。
...
- 处理抽取失败、正文为空、来源缺失等情况。
- 返回明确的粘贴正文提示和模式降级信息。
实现备注：当前 URL 输入仍只支持 `mock_fetch_result` 或保守 fallback，真实正文抽取尚未开始；这是把 URL 输入从“界面支持”推进到“真实可用”的第一阻塞点。

本轮执行任务：
- 明确 URL 抽取策略与边界，只处理可直接抓取的 HTML 页面，不扩到真实检索、登录页、反爬绕过或时间线联动。
- 新增轻量 URL 抽取服务，产出可供 `InputNormalizer` 使用的结构化结果，并在失败时保留清晰的 fallback reason。
- 把 URL 抽取结果接入 `AnalyzePipeline -> InputNormalizer -> ReportBuilder` 主链路，让 `title / summary / source_name / published_at` 在可抽取时进入最终事件信息。
- 补齐 URL 抽取成功、抽取失败、超时异常 fallback、文本输入不回归的后端测试，并同步更新后端说明文档。

执行步骤：
- 盘点现有 URL 输入分支、fallback 条件和风险提示，确定新增结构化抽取结果的模型边界。
- 实现独立 URL 抽取模块，优先抓取 HTML 页面的标题、正文、摘要、来源和发布时间，并统一状态码与错误原因。
- 在 `AnalyzePipeline` 中为 URL 输入接入抽取调用，把结果传给 `InputNormalizer`，确保失败时仍输出保守结果而不抛异常。
- 调整 `InputNormalizer` / `ReportBuilder` 的 URL fallback 文案和字段落点，让成功路径可用、失败路径可解释。
- 补测试并跑可复现验证，最后回写 `tasks/cluster-c-api-foundation.md` 与后端实现文档。
