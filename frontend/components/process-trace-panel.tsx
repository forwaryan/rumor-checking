    ...
    steps: [
      {
        ...
        title: "接收输入",
        status: "completed",
        summary: "当前输入已经进入页面状态，准备发起 analyze 请求。",
      ...
      },
      {
        ...
        title: "请求发出",
        status: "completed",
        summary: "前端已向后端发起 analyze 请求，正在等待返回。",
        details: ["当前页面还没有拿到 report，链路细节会在返回后自动补齐。"],
      },
      {
        stage_key: "response_pending",
        title: "等待结果",
        status: "warning",
        summary: "后端尚未返回完整结果，因此其余步骤暂时不可见。",
        details: [],
      },
    ],
  };
    ...
    steps: [
      {
        ...
        details: [`原始输入：${preview(requestInput || "无")}`, `模式：${report.mode}`],
      },
      {
      ...
      },
      {
        ...
        summary: `当前 report 共输出 ${report.claim_results.length} 条 claim，其中 ${decisiveCount} 条已拿到非“证据不足”判定。`,
      ...
      },
      {
        ...
        summary: report.timeline.length ? `当前 report 还原了 ${report.timeline.length} 个时间线节点。` : "当前 report 还没有稳定时间线节点。",
      ...
      },
      {
        ...
        details: [`来源：${report.provenance?.source_type ?? "unknown"}`, `总结：${preview(report.final_summary, 128)}`],
      },
    ],
  };
}
...
export function ProcessTracePanel({ report, request, status }: ProcessTracePanelProps) {
    ...
    return (
      <section className="panel panel--trace">
        ...
        <p className="empty-state">
          这里会按步骤展示从输入、检索、收束、判定到最终报告输出的整条链路，用来定位问题；默认保持折叠，避免压过主结论。
        </p>
      </section>
    );
  }
  ...
  return (
    <section className="panel panel--trace">
      ...
      <details className="trace-disclosure" open={status === "submitting"}>
        <summary className="trace-disclosure__summary">
          <span>展开调试链路</span>
          <span className="trace-disclosure__hint">默认折叠，避免抢占主结果</span>
        </summary>
